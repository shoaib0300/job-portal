...
function onboarding_flow_steps(): array
{
    return [
        ...
        [
            'id' => 'copy',
            'label' => 'New job',
            'caption' => kaamfit_brand_name() . ' copies Master into a Job CV and job letter per application.',
            'href' => '/tailor',
            'icon' => 'apply',
        ],
        [
...
function onboarding_render_hero(bool $collapsible = false, bool $startOpen = true): void
{
    ...
    ?>
    <div class="kf-flow-hero-wrap<?= $collapsible ? ' kf-flow-hero-wrap--collapsible' : '' ?>" data-kf-flow-hero>
      <?php if ($collapsible): ?>
        <button class="kf-flow-hero-toggle btn btn-link text-decoration-none w-100 text-start p-0 mb-2"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#<?= $collapseId ?>"
                aria-expanded="<?= $open ? 'true' : 'false' ?>"
                aria-controls="<?= $collapseId ?>">
          <span class="settings-panel-title mb-0">How <?= App::e(kaamfit_brand_name()) ?> works</span>
          <span class="small text-secondary ms-2">— 30 second overview</span>
        </button>
        <div class="collapse<?= $open ? ' show' : '' ?>" id="<?= $collapseId ?>">
      <?php endif; ?>
      <div class="kf-flow-hero card shadow-sm" data-active-step="0">
        <div class="card-body">
          <div class="kf-flow-hero-stage" role="img" aria-label="Workflow diagram">
            <svg class="kf-flow-hero-svg" viewBox="0 0 720 120" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path class="kf-flow-path" d="M60 60 H660" fill="none" stroke-width="3" opacity="0.2"/>
              <path class="kf-flow-path-anim" d="M60 60 H660" fill="none" stroke-width="3" opacity="0.85"/>
              <g class="kf-flow-dot-wrap" transform="translate(60, 0)" data-flow-dot-wrap>
                <circle class="kf-flow-dot" cx="0" cy="60" r="8"/>
              </g>
              <?php
              ...
              foreach ($steps as $i => $step):
                  $x = $xs[$i];
              ?>
                <g class="kf-flow-node<?= $i === 0 ? ' is-active' : '' ?>" data-flow-step="<?= App::e($step['id']) ?>">
                  <rect x="<?= $x - 28 ?>" y="28" width="56" height="64" rx="10" class="kf-flow-node-box"/>
                  <text x="<?= $x ?>" y="72" text-anchor="middle" class="kf-flow-node-num"><?= $i + 1 ?></text>
                </g>
              <?php endforeach; ?>
            </svg>
          </div>
          <div class="kf-flow-hero-tabs" role="tablist" aria-label="Workflow steps">
            <?php foreach ($steps as $i => $step): ?>
              <button type="button"
                      class="kf-flow-tab<?= $i === 0 ? ' is-active' : '' ?>"
                      role="tab"
                      id="kf-flow-tab-<?= App::e($step['id']) ?>"
                      aria-selected="<?= $i === 0 ? 'true' : 'false' ?>"
                      aria-controls="kf-flow-panel-<?= App::e($step['id']) ?>"
                      data-flow-tab="<?= App::e($step['id']) ?>">
                <?= App::e($step['label']) ?>
              </button>
            <?php endforeach; ?>
          </div>
          <div class="kf-flow-hero-panels">
            <?php foreach ($steps as $i => $step): ?>
              <div class="kf-flow-panel<?= $i === 0 ? ' is-active' : '' ?>"
                   id="kf-flow-panel-<?= App::e($step['id']) ?>"
                   role="tabpanel"
                   aria-labelledby="kf-flow-tab-<?= App::e($step['id']) ?>"
                   data-flow-panel="<?= App::e($step['id']) ?>"
                   <?= $i !== 0 ? 'hidden' : '' ?>>
                <p class="kf-flow-caption mb-2"><?= App::e($step['caption']) ?></p>
                <a class="btn btn-sm btn-outline-primary" href="<?= App::e($step['href']) ?>">Go to <?= App::e($step['label']) ?></a>
              </div>
            <?php endforeach; ?>
          </div>
          <p class="kf-flow-live small text-secondary mb-0 mt-2" aria-live="polite" data-flow-live><?= App::e($steps[0]['label']) ?></p>
      ...
      <?php if ($collapsible): ?>
        ...
        <?php if ($seen): ?>
          <p class="kf-onboard-reopen mb-0 mt-2">
...
{
    $cls = 'kf-flow-mini' . ($animated ? ' kf-flow-mini--animated' : '');
    $uid = onboarding_mini_uid();
    ?>
    <div class="<?= $cls ?>" data-kf-flow-mini="<?= App::e($variant) ?>" aria-hidden="true">
      <?php if ($variant === 'resume' || $variant === 'master' || $variant === 'copy'): ?>
        <svg viewBox="0 0 160 96" class="kf-flow-mini-svg">
          ...
          <path class="kf-mini-arrow" d="M58 45 H88" stroke="var(--kf-accent, #0d7377)" stroke-width="2.5" stroke-linecap="round" marker-end="url(#<?= $uid ?>-arrow)"/>
          <g class="kf-mini-copy-wrap">
            <rect x="94" y="22" width="40" height="54" rx="5" fill="var(--kf-accent, #0d7377)" opacity="0.25" stroke="var(--kf-accent, #0d7377)" stroke-width="2.5"/>
            <text x="114" y="54" text-anchor="middle" font-size="9" font-weight="800" fill="currentColor">Job CV</text>
          </g>
          <defs><marker id="<?= $uid ?>-arrow" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="var(--kf-accent, #0d7377)"/></marker></defs>
        </svg>
      <?php elseif ($variant === 'jobs' || $variant === 'find'): ?>
        <svg viewBox="0 0 160 96" class="kf-flow-mini-svg">
          <g class="kf-mini-search-wrap">
      ...
      <?php elseif ($variant === 'cover'): ?>
        <svg viewBox="0 0 160 96" class="kf-flow-mini-svg">
          ...
          <path class="kf-mini-arrow" d="M60 42 H84" stroke="var(--kf-accent, #0d7377)" stroke-width="2.5" stroke-linecap="round"/>
          <g class="kf-mini-copy-wrap">
            <rect x="90" y="26" width="44" height="36" rx="5" fill="var(--kf-accent, #0d7377)" opacity="0.25" stroke="var(--kf-accent, #0d7377)" stroke-width="2.5"/>
            <path d="M94 36 L112 44 L130 36" fill="none" stroke="var(--kf-accent, #0d7377)" stroke-width="1.5"/>
          </g>
        </svg>
      <?php elseif ($variant === 'edit'): ?>
        <svg viewBox="0 0 160 96" class="kf-flow-mini-svg">
          ...
          <line class="kf-mini-edit-line" x1="36" y1="38" x2="124" y2="38" stroke="var(--kf-accent, #0d7377)" stroke-width="3" stroke-linecap="round"/>
      ...
      <?php elseif ($variant === 'pdf'): ?>
        <svg viewBox="0 0 160 96" class="kf-flow-mini-svg">
          ...
          <text x="80" y="54" text-anchor="middle" font-size="13" font-weight="800" fill="var(--kf-accent, #0d7377)">PDF</text>
        </svg>
      <?php elseif ($variant === 'track'): ?>
        <svg viewBox="0 0 160 96" class="kf-flow-mini-svg">
          ...
          <circle class="kf-mini-track-dot" cx="46" cy="48" r="9" fill="var(--kf-accent, #0d7377)" opacity="0.35"/>
          <circle class="kf-mini-track-dot" cx="80" cy="48" r="9" fill="var(--kf-accent, #0d7377)" opacity="0.55"/>
          <circle class="kf-mini-track-dot" cx="114" cy="48" r="9" fill="var(--kf-accent, #0d7377)"/>
        </svg>
      <?php endif; ?>
    </div>
    <?php
...
function onboarding_render_banner(string $section): void
{
    ...
    ?>
    <aside class="kf-onboard-banner card shadow-sm mb-3" data-onboarding-banner="<?= App::e($section) ?>">
